...
// Script block 1
  ...
  const menuModules = [
    {id:'dashboard', label:'Dashboards', icon:'â–¦'}, {id:'banking', label:'Banking', icon:'â—‰'}, {id:'transactions', label:'Transactions', icon:'â‡„'}, {id:'accounting', label:'Accounting', icon:'â–¤'},
    {id:'sales', label:'Sales', icon:'â†—'}, {id:'customers', label:'Customers & Leads', icon:'â˜˜'}, {id:'expenses', label:'Expenses', icon:'â–¸'}, {id:'vendors', label:'Vendors', icon:'â–¡'},
    {id:'reports', label:'Reports', icon:'â˜·'}, {id:'inventory', label:'Inventory', icon:'â—¼'}, {id:'projects', label:'Projects', icon:'â—†'},
    {id:'time', label:'Time', icon:'â—·'}, {id:'payroll', label:'Payroll', icon:'â™¢'}, {id:'taxes', label:'Taxes', icon:'â—–'}, {id:'settings', label:'Settings', icon:'âš™'}
  ];
  ...
  function accountLabel(id){ const a=getAccount(id); return `${a.code} Â· ${a.name}`; }
  ...
  function renderMenu(){
    ...
    list.innerHTML = menuModules.filter(m=>visible.has(m.id)).map(m=>`<button class="nav-item ${currentPage===m.id?'active':''}" data-nav="${m.id}"><span class="dot">${m.icon}</span>${escapeHTML(m.label)}<span class="nav-chevron">â€º</span></button>`).join('');
  }
  function renderModulePills(){
    const mods = [
      {label:'Banking', icon:'â—‰', nav:'banking'}, {label:'Transactions', icon:'â‡„', nav:'transactions'}, {label:'Accounting', icon:'â–¤', nav:'accounting'}, {label:'Sales & Get Paid', icon:'â–£', nav:'sales'},
      {label:'Expenses & Pay Bills', icon:'â€º', nav:'expenses'}, {label:'Customers', icon:'âœ¿', nav:'customers'}, {label:'Reports', icon:'â˜·', nav:'reports'}, {label:'Inventory', icon:'â—¼', nav:'inventory'}
    ];
  ...
  function renderBusinessFeed(t){
    ...
    const cards = [
      {icon:'â—‰', title:'Banking Agent', text:`${unreviewed} bank-feed transactions need review, matching, or categorization.`, action:'Review banking', nav:'banking'},
      {icon:'âœ“', title:'Accounting Agent', text:`Trial balance check: ${tb.ok?'balanced':'needs review'} (${money(Math.abs(tb.diff))} difference).`, action:'Open accounting', nav:'accounting'},
      {icon:'$', title:'Payments Agent', text:`Open A/R is ${money(t.ar)}. Overdue exposure is ${money(t.overdue)}.`, action:'Review A/R', nav:'sales'}
    ];
    document.getElementById('businessFeed').innerHTML = `<div class="feed-row">${cards.map(c=>`<div class="feed-card"><span class="menu">â‹®</span><div class="feed-title"><span class="feed-badge">${c.icon}</span>${c.title}</div><p>${escapeHTML(c.text)}</p><button class="btn soft" data-nav="${c.nav}">${c.action}</button></div>`).join('')}</div>`;
  }
  ...
  function shortenBankDescription(description, maxLen=28){
    ...
    if(!raw) return '';
    const firstSegment = raw.split(/\s+[-â€“â€”]\s+/)[0].trim();
    let shortText = firstSegment.length >= 8 ? firstSegment : raw;
    if(shortText.length > maxLen) shortText = shortText.slice(0, maxLen - 1).trimEnd() + 'â€¦';
    return shortText;
  }
  ...
  function renderPostingPreview(tx){
    ...
    return `<div class="posting-preview"><div class="posting-line posting-head"><span>Account</span><span>Debit</span><span>Credit</span></div>${rows.map(r=>`<div class="posting-line"><span>${escapeHTML(accountLabel(r.accountId))}</span><span>${r.debit?money(r.debit):'â€”'}</span><span>${r.credit?money(r.credit):'â€”'}</span></div>`).join('')}</div>`;
  }
  function renderBankTransactionRows(txs){
    return txs.map(tx=>{
      ...
      return [`<span class="small muted">${tx.date}</span>`,
        `<div class="tx-desc"><strong>${escapeHTML(tx.description)}</strong><span class="muted small">${tx.id} Â· ${escapeHTML(getBank(tx.bankAccountId).name)}</span></div>`,
  ...
  function renderTransactions(){
    ...
    const rows = [
      ...
      ...state.invoices.map(i=>({date:i.date, source:'Invoice', ref:i.id, name:getCustomer(i.customerId).name, amount:invoiceTotal(i), status:i.status, action:openAmount(i)>0?`<button class="btn square" data-action="mark-paid" data-id="${i.id}">Receive</button>`:'â€”'})),
      ...state.expenses.map(e=>({date:e.date, source:'Expense', ref:e.id, name:getVendor(e.vendorId).name, amount:-expenseTotal(e), status:e.paymentMethod, action:'â€”'})),
      ...state.bills.map(b=>({date:b.date, source:'Bill', ref:b.id, name:getVendor(b.vendorId).name, amount:-billTotal(b), status:b.status, action:billOpenAmount(b)>0?`<button class="btn square" data-action="pay-bill" data-id="${b.id}">Pay</button>`:'â€”'})),
      ...state.payments.map(p=>({date:p.date, source:'Payment', ref:p.id, name:getCustomer(p.customerId).name, amount:p.amount, status:'Posted', action:'â€”'}))
  ...
  function renderReports(){
    ...
    `<div class="card table-card" style="margin-top:16px"><div class="toolbar"><div><h3 style="margin:0">Trial Balance</h3><div class="muted small">Status: ${tb.ok?'balanced':'review needed'} Â· Difference ${money(Math.abs(tb.diff))}</div></div></div>${table(['Code','Account','Type','Debit','Credit'], state.chartOfAccounts.map(a=>{ const b=bal[a.id]||{net:0}; const d=b.net>0?b.net:0, c=b.net<0?-b.net:0; return [a.code,escapeHTML(a.name),a.type,d?money(d):'â€”',c?money(c):'â€”']; }).filter(r=>r[3]!=='â€”'||r[4]!=='â€”'))}</div>`+
    `<div class="grid two" style="margin-top:16px"><div class="card table-card"><div class="toolbar"><h3 style="margin:0">Accounts Receivable Aging</h3></div>${table(['Invoice','Customer','Due','Status','Open'], state.invoices.filter(i=>openAmount(i)>0).map(i=>[i.id,escapeHTML(getCustomer(i.customerId).name),i.dueDate,tagForStatus(i.status),money(openAmount(i))]))}</div><div class="card table-card"><div class="toolbar"><h3 style="margin:0">Accounts Payable Aging</h3></div>${table(['Bill','Vendor','Due','Status','Open'], state.bills.filter(b=>billOpenAmount(b)>0).map(b=>[b.id,escapeHTML(getVendor(b.vendorId).name),b.dueDate,tagForStatus(b.status),money(billOpenAmount(b))]))}</div></div>`;
  }
  ...
  function bankOptions(){ return state.bankAccounts.map(b=>`<option value="${b.id}">${escapeHTML(b.name)} Â· ${money(normalBalance(b.accountId))}</option>`).join(''); }
  function accountOptions(filterTypes=null){ return state.chartOfAccounts.filter(a=>!filterTypes || filterTypes.includes(a.type)).map(a=>`<option value="${a.id}">${a.code} Â· ${escapeHTML(a.name)}</option>`).join(''); }
  function productOptions(){ return state.products.map(p=>`<option value="${p.id}" data-price="${p.price}" data-account="${p.incomeAccountId||'4000'}">${escapeHTML(p.name)} Â· ${money(p.price)}</option>`).join(''); }
  ...
  function exportData(){ const blob = new Blob([JSON.stringify(state,null,2)], {type:'application/json'}); const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='smartbooks-company-data.json'; a.click(); URL.revokeObjectURL(url); showToast('Backup export started.'); }
