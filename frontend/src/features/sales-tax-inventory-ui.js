...
var dashboardShortcutDefaults = ['sales','expenses','banking','reports','customers','inventory','taxes','projects','time','accounting','transactions'];
var v815FeedCategories = [];
var v815DefaultFeedCategories = [];
var v816FeedCategories = [
  {id:'banking', label:'Banking', icon:'banking', desc:'Bank review, matching, clearing, and reconciliation.', module:'banking'},
  {id:'sales', label:'Sales & Invoices', icon:'sales', desc:'Open receivables, overdue invoices, payments, and estimates.', module:'sales'},
  {id:'expenses', label:'Expenses & Bills', icon:'expenses', desc:'Vendor bills, payments, approvals, and due-date reminders.', module:'expenses'},
  {id:'taxes', label:'Taxes', icon:'taxes', desc:'GST/HST return, credit/payable, and tax-code review.', module:'taxes'},
  {id:'inventory', label:'Products & Services', icon:'inventory', desc:'Low stock, inventory attention, and item setup.', module:'inventory', requiresInventoryMode:true},
  {id:'matching', label:'Purchase Matching', icon:'transactions', desc:'PO, receiving, vendor bill, and three-way match alerts.', module:'inventory', requiresPurchaseMode:true},
  {id:'accounting', label:'Accounting', icon:'accounting', desc:'Trial balance, journal review, and accounting checks.', module:'accounting'},
  {id:'cash', label:'Cash Flow', icon:'cash', desc:'Cash position, net movement, and bank balances.', module:'banking'},
  {id:'setup', label:'Setup & Admin', icon:'check', desc:'Company setup, preferences, and backup reminders.', module:'settings'}
];
var v816CategoryMap = {bank:'banking', overdue:'sales', ap:'expenses', taxes:'taxes', stock:'inventory', match:'matching', balanced:'accounting', cash:'cash', none:'setup'};
var v816DefaultCategories = v816FeedCategories.map(c=>c.id);
  ...
  renderMenu = function(){
    ...
    const v4Modules = [
      {id:'dashboard', label:'Dashboards', icon:'▦'}, {id:'setup', label:'Setup Checklist', icon:'✓'}, {id:'apps', label:'My Apps', icon:'▩'},
      {id:'banking', label:'Banking', icon:'◉'}, {id:'transactions', label:'Transactions', icon:'⇄'}, {id:'accounting', label:'Accounting', icon:'▤'},
      {id:'sales', label:'Sales & Get Paid', icon:'↗'}, {id:'customers', label:'Customers & Leads', icon:'☘'}, {id:'expenses', label:'Expenses', icon:'▸'}, {id:'vendors', label:'Vendors', icon:'□'},
      {id:'reports', label:'Reports', icon:'☷'}, {id:'inventory', label:'Inventory', icon:'◼'}, {id:'projects', label:'Projects', icon:'◆'},
      {id:'time', label:'Time', icon:'◷'}, {id:'payroll', label:'Payroll & HR', icon:'♢'}, {id:'taxes', label:'Taxes', icon:'◖'}, {id:'settings', label:'Settings', icon:'⚙'}
    ];
    list.innerHTML = v4Modules.filter(m=>visible.has(m.id)).map(m=>`<button class="nav-item ${currentPage===m.id?'active':''}" data-nav="${m.id}"><span class="dot">${m.icon}</span>${escapeHTML(m.label)}<span class="nav-chevron">›</span></button>`).join('');
  };

  const originalRenderModulePills = renderModulePills;
  renderModulePills = function(){
    const mods = [
      {label:'Accounting', icon:'▤', nav:'accounting'}, {label:'Expenses', icon:'▸', nav:'expenses'}, {label:'Sales', icon:'↗', nav:'sales'},
      {label:'Customers', icon:'✿', nav:'customers'}, {label:'Setup', icon:'✓', nav:'setup'}, {label:'Banking', icon:'◉', nav:'banking'},
      {label:'Time', icon:'◷', nav:'time'}, {label:'Projects', icon:'◆', nav:'projects'}, {label:'Inventory', icon:'◼', nav:'inventory'}, {label:'Taxes', icon:'◖', nav:'taxes'}
    ];
  ...
  renderApps = function(){
    ...
    const apps = [
      ['Accounting','▤','Chart of accounts, journal entries, trial balance, audit trail.','accounting'],
      ['Expenses & Pay Bills','▸','Expenses, bills, vendor payments, and A/P workflow.','expenses'],
      ['Sales & Get Paid','↗','Invoices, payment links, recurring payments, sales orders, channels, payouts.','sales'],
      ['Customer Hub','☘','Customer records, open balances, estimates, and invoices.','customers'],
      ['Banking','◉','Bank feed review, matching, categorization, and reconciliation.','banking'],
      ['Reports','☷','Profit & Loss, Balance Sheet, Trial Balance, A/R and A/P aging.','reports'],
      ['Inventory','◼','Products, services, quantity, and income accounts.','inventory'],
      ['Time','◷','Billable time entries by team member and customer.','time'],
      ['Projects','◆','Project budgets, actual costs, revenue, and profitability.','projects'],
      ['Taxes','◖','GST/HST collected, ITCs, and net payable.','taxes'],
      ['Setup Checklist','✓','Guided setup for company, sales, banking, taxes, and dashboard.','setup'],
      ['Settings','⚙','Company profile, menu customization, dashboard controls, and data export.','settings']
    ];
  ...
  function taxCodeOptions(selected='GST5'){ return (state.taxCodes||[]).filter(c=>c.active!==false).map(c=>`<option value="${c.id}" data-rate="${num(c.rate)}" ${c.id===selected?'selected':''}>${escapeHTML(c.code)} · ${num(c.rate)}%</option>`).join(''); }
  ...
  renderBusinessFeed = function(t){
    ...
    const html = `<div class="feed-card"><span class="menu">⋮</span><div class="feed-title"><span class="feed-badge">%</span>Tax Agent</div><p>${exceptions} tax exceptions need review. Current net sales tax balance is ${money(tax.balance)}.</p><button class="btn soft" data-nav="taxes">Open Tax Center</button></div>`;
  ...
  function invoiceItemRows(inv, settings){
    ...
    const rows = (inv.items||[]).map(l=>[showSku?escapeHTML(l.sku||'—'):'', escapeHTML(l.product||'Service'), `<div>${escapeHTML(l.desc||'Item')}</div>${l.taxCodeId?`<div class="muted-line">Tax code: ${escapeHTML(invoiceTaxCodeLabel(l.taxCodeId))}</div>`:''}`, num(l.qty), money(num(l.rate)), money(num(l.amount ?? num(l.qty)*num(l.rate))), showTax?money(num(l.taxAmount||0)):''].filter((_,idx)=> idx!==0 || showSku).filter((_,idx,arr)=>true));
  ...
  invoiceItemRows = function(inv, settings){
    ...
    const showTax = settings.showTaxColumn !== false;
    const lineCols = [];
    if(showSku) lineCols.push({h:'SKU', v:l=>escapeHTML(l.sku||'—'), num:false});
  ...
  function inventoryProductOptions(selected){ return inventoryProducts().map(p=>`<option value="${p.id}" ${p.id===selected?'selected':''}>${escapeHTML(p.sku||p.id)} · ${escapeHTML(p.name)} · available ${num(stockAvailable(p.id))}</option>`).join(''); }
  ...
  renderInventory = function(){
    ...
    if(active==='overview') body=inventoryKPIs()+`<div class="grid two"><div class="card"><h3>Connected workflow</h3><div class="mini-flow"><span>Vendor</span><b>→</b><span>PO</span><b>→</b><span>Receive</span><b>→</b><span>Bill</span><b>→</b><span>Pay</span></div><div class="mini-flow"><span>Customer</span><b>→</b><span>SO</span><b>→</b><span>Reserve</span><b>→</b><span>Fulfill</span><b>→</b><span>Invoice</span></div><p class="muted">Products, vendors, customers, inventory value, COGS, A/P, A/R, and tax coding are connected.</p></div><div class="card"><h3>Low-stock alerts</h3>${lowStockProducts().map(p=>`<div class="report-line"><span>${escapeHTML(p.name)} <span class="muted small">${escapeHTML(p.sku)}</span></span><strong>${num(stockAvailable(p.id))} available</strong></div>`).join('') || '<div class="empty">No low-stock alerts.</div>'}</div></div>`;
    ...
    if(active==='receiving'){ const rows=[...state.purchaseOrders.filter(po=>num(po.receivedQty)<num(po.qty)).map(po=>({status:'Open',ref:po.id,product:escapeHTML(getProduct(po.productId).name),qty:num(num(po.qty)-num(po.receivedQty)),date:po.expectedDate,cost:'—',action:`<button class="btn square" data-action="receive-po" data-id="${po.id}">Receive remaining</button>`})),...state.receivingRecords.map(r=>({status:'Received',ref:`${r.id}<div class="muted small">PO ${escapeHTML(r.poId)}</div>`,product:escapeHTML(getProduct(r.productId).name),qty:num(r.qty),date:r.date,cost:money(r.unitCost),action:'—'}))].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))); body=`<div class="card table-card"><div class="toolbar"><div><h3 style="margin:0">Receiving activity</h3><div class="muted small">Open receiving work and completed receipts in one inventory workflow queue.</div></div></div>${table(['Status','Reference','Product','Qty','Expected / received','Unit cost','Action'],rows.map(r=>[tagForStatus(r.status),r.ref,r.product,r.qty,r.date,r.cost,r.action]))}</div>`; }
  ...
  function sellableItemOptions(selected){ return sellableItems().map(p=>`<option value="${p.id}" ${p.id===selected?'selected':''} data-price="${num(p.price)}" data-account="${p.incomeAccountId||'4000'}">${escapeHTML(p.sku||p.id)} · ${escapeHTML(p.name)} · ${escapeHTML(itemType(p))} · ${money(p.price)}</option>`).join(''); }
  ...
  function renderItemTable(items, type){
    if(type==='Service') return table(['Service','Category','Billing unit','Rate','Income account','Tax code','Revenue','Est. margin','Actions'], items.map(p=>[`<strong>${escapeHTML(p.name)}</strong><div class="muted small">${escapeHTML(p.sku||p.id)} · ${escapeHTML(p.description||'')}</div>`,escapeHTML(p.serviceCategory||'Service'),escapeHTML(p.salesUnit||'hour'),money(p.price),escapeHTML(accountLabel(p.incomeAccountId||'4000')),escapeHTML(getTaxCode(p.taxCodeId||'GST5').code),money(lineRevenueForProduct(p)),money(serviceProfitForProduct(p)),`<button class="btn square" data-action="edit-item" data-id="${p.id}">Edit</button>`]));
  ...
  renderInventory=function(){
    ...
    let body='';
    if(active==='overview') body=v71Metrics()+`<div class="grid two"><div class="card"><h3>Products & Services Center</h3><div class="mini-flow"><span>Services</span><b>→</b><span>Estimate / SO</span><b>→</b><span>Invoice</span><b>→</b><span>Service revenue</span></div><div class="mini-flow"><span>Inventory</span><b>→</b><span>PO</span><b>→</b><span>Receive</span><b>→</b><span>Bill</span><b>→</b><span>COGS on sale</span></div><p class="muted">Services are separated from inventory. Services do not show quantity, reorder point, receiving, inventory asset, or COGS movement.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn primary" data-modal="serviceItem">Add service</button><button class="btn" data-modal="product">Add product/item</button></div></div><div class="card"><h3>Item type rules</h3><div class="report-line"><span>${itemTypePill({type:'Service'})} Service</span><strong>No inventory</strong></div><div class="report-line"><span>${itemTypePill({type:'Product',trackInventory:true})} Inventory</span><strong>Asset + COGS</strong></div><div class="report-line"><span>${itemTypePill({type:'Non-Inventory'})} Non-Inventory</span><strong>No stock balance</strong></div><div class="report-line"><span>${itemTypePill({type:'Bundle'})} Bundle</span><strong>Package</strong></div></div></div><div class="card table-card" style="margin-top:16px"><div class="toolbar"><h3 style="margin:0">Service profitability summary</h3><button class="btn" data-modal="serviceItem">Add service</button></div>${renderItemTable(serviceItems(),'Service')}</div>`;
  ...
  renderMenu=function(){
    v71RenderMenuBase();
    document.querySelectorAll('[data-nav="inventory"]').forEach(btn=>{ if(btn.classList.contains('nav-item')) btn.innerHTML='<span class="dot">◼</span>Products & Services<span class="nav-chevron">›</span>'; });
  };
  ...
  function modeSellableItemOptions(selected){ const items=modeSellableItems(); if(!items.length) return '<option value="">No active items available for the selected business setup</option>'; return items.map(p=>`<option value="${p.id}" ${p.id===selected?'selected':''} data-price="${num(p.price)}" data-account="${p.incomeAccountId||'4000'}">${escapeHTML(p.sku||p.id)} · ${escapeHTML(p.name)} · ${escapeHTML(itemType(p))} · ${money(p.price)}</option>`).join(''); }
  function modeInventoryProductOptions(selected){ const items=modeInventoryProducts(); if(!items.length) return '<option value="">No inventory products available</option>'; return items.map(p=>`<option value="${p.id}" ${p.id===selected?'selected':''}>${escapeHTML(p.sku||p.id)} · ${escapeHTML(p.name)} · available ${num(stockAvailable(p.id))}</option>`).join(''); }
  ...
  function matchingBody(){ return `<div class="card table-card"><div class="toolbar"><div><h3 style="margin:0">Three-way match: PO → Receiving → Bill</h3><div class="muted small">Compare ordered quantity, received quantity, bill source, amount, and match status.</div></div></div>${table(['PO','Vendor','Product','Ordered','Received','Bill','PO total','Match status','Detail'],state.purchaseOrders.map(po=>{const m=poMatchStatus(po); return [po.id,escapeHTML(getVendor(po.vendorId).name),escapeHTML(getProduct(po.productId).name),num(po.qty),num(po.receivedQty),po.billId||'—',money(poTotal(po)),`<span class="tag ${m.cls}">${m.label}</span>`,escapeHTML(m.detail)]}))}</div>`; }
  function movementsBody(){ return `<div class="card table-card"><div class="toolbar"><div><h3 style="margin:0">Inventory movements</h3><div class="muted small">Receipts, adjustments, fulfillment, and COGS movements.</div></div><button class="btn primary" data-modal="inventoryAdjust">Adjust inventory</button></div>${table(['Movement','Date','Type','Product','Qty','Unit cost','COGS','Source','Memo'],state.inventoryMovements.map(m=>[m.id,m.date,escapeHTML(m.type),escapeHTML(getProduct(m.productId).name),num(m.qty),money(m.unitCost),money(m.cogsAmount),escapeHTML(m.sourceId||m.poId||m.soId||'—'),escapeHTML(m.memo||'')]))}</div>`; }
  ...
  const masterModuleRegistry = [
    {id:'dashboard', label:'Dashboards', icon:'▦', locked:true},
    {id:'setup', label:'Setup Checklist', icon:'✓'},
    {id:'apps', label:'My Apps', icon:'▩'},
    {id:'banking', label:'Banking', icon:'◉'},
    {id:'transactions', label:'Transactions', icon:'⇄'},
    {id:'accounting', label:'Accounting', icon:'▤'},
    {id:'sales', label:'Sales & Get Paid', icon:'↗'},
    {id:'customers', label:'Customers & Leads', icon:'☘'},
    {id:'expenses', label:'Expenses & Pay Bills', icon:'▸'},
  ...
  const alwaysVisibleModules = new Set(['dashboard']);
  ...
  function ensureV74State(){
    ...
    if(!menuModules.some(m=>m.id==='setup')) menuModules.splice(1,0,{id:'setup', label:'Setup Checklist', icon:'✓'});
    if(!menuModules.some(m=>m.id==='apps')) menuModules.splice(2,0,{id:'apps', label:'My Apps', icon:'▩'});
  ...
  function modalRequiredModules(type){
    const map = {
      ...
      customize:[], customizeDashboard:[], company:[]
    };
    return map[type] || [];
  }
  ...
  renderMenu = function(){
    ...
    list.innerHTML = masterModuleRegistry
      ...
      .map(m=>`<button class="nav-item ${currentPage===m.id?'active':''}" data-nav="${m.id}"><span class="dot">${m.icon}</span>${escapeHTML(m.label)}<span class="nav-chevron">›</span></button>`)
      .join('');
  };
  renderModulePills = function(){
    ensureV74State();
  ...
  renderDashboard = function(){
    ...
    page.innerHTML = `
      ...
      <div class="feed-header"><h3>✦ Business Feed</h3><button class="btn soft" data-action="refresh-dashboard">View all</button></div>
      <div id="businessFeed"></div>
      ${quickActionsV8()}
      <div class="section-header"><div><h2>Business at a glance</h2><p>Clean summary of receivables, deposits, bank balances, profit, expenses, and setup items.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-modal="customizeDashboard">⚙ Customize</button><button class="btn" data-action="toggle-privacy">◉ Privacy</button><button class="btn square" data-action="refresh-dashboard">↻ Refresh</button></div></div>
      <div class="v8-dashboard-grid">
        ...
        <div class="v8-side-col">
          ...
          <div class="v8-add-widget-card"><div class="plus">＋</div><strong>Add widgets</strong><p class="muted">Show sales, cash flow, A/R, A/P, inventory, or tax cards from Customize.</p><button class="btn" data-modal="customizeDashboard">Customize dashboard</button></div>
        </div>
      </div>`;
    renderModulePills();
    const t=totals();
  ...
  renderBusinessFeed = function(t){
    ...
    const cards = [
      {icon:'▣', title:'Accounting done', text:`Trial balance is ${tb.ok?'balanced':'out by '+money(Math.abs(tb.diff))}. ${state.bankTransactions.filter(x=>x.posted).length} bank transactions are posted.`, action:'Open accounting', nav:'accounting'},
      ...
      {icon:'◉', title:'Banking review', text:`${unreviewed} imported-style bank transaction${unreviewed===1?'':'s'} need matching, clearing, or categorization.`, action:'Review banking', nav:'banking'},
      {icon:'↗', title:'Cash flow insight', text:`Current bank position is ${money(t.bank)}. Net cash flow is based on recorded payments, deposits, expenses, and bill payments.`, action:'View banking', nav:'banking'},
      {icon:'◼', title:'Low stock detected', text:`${lowStock} inventory item${lowStock===1?'':'s'} are at or below reorder point.`, action:'Open items', nav:'inventory'},
      {icon:'◖', title:'Tax return ready', text:`Estimated net tax ${taxNet>=0?'payable':'recoverable'} is ${money(Math.abs(taxNet))}.`, action:'Prepare return', nav:'taxes'},
      {icon:'▸', title:'Bills due', text:`${dueBills.length} open vendor bill${dueBills.length===1?'':'s'} require review or payment scheduling.`, action:'Review bills', nav:'expenses'}
    ].filter(c=>!c.nav || isModuleVisible(c.nav)).slice(0,4);
    const el=document.getElementById('businessFeed'); if(!el) return;
    el.innerHTML = `<div class="feed-row v8-feed-row">${cards.map(c=>`<div class="feed-card v8-feed-card"><span class="menu">⋮</span><div><div class="feed-title"><span class="feed-badge">${c.icon}</span>${escapeHTML(c.title)}</div><p>${escapeHTML(c.text)}</p></div><button class="btn soft" data-nav="${c.nav}">${escapeHTML(c.action)}</button></div>`).join('')}<div class="feed-card v8-feed-card"><div><div class="feed-title"><span class="feed-badge">✦</span>More ready for you</div><p>Use Customize to choose dashboard widgets and menu modules for your business workflow.</p></div><button class="btn soft" data-modal="customizeDashboard">Customize</button></div></div>`;
  };
  ...
  function reportDataV81(id){
    ...
    if(id==='trial-balance'){
      const rows=state.chartOfAccounts.map(a=>{ const b=bal[a.id]||{net:0}; return [a.code,a.name,a.type,b.net>=0?money(b.net):'—',b.net<0?money(-b.net):'—']; });
      const debit=rows.reduce((s,r)=>s+(r[3]==='—'?0:0),0); const tb=trialBalanceStatus();
  ...
  function injectV85TopbarStyles(){
    ...
    const style=document.createElement('style'); style.id='v85-topbar-styles'; style.textContent=`
      ...
      .top-panel-body{padding:12px 14px 16px}.top-panel-close{border:0;background:transparent;border-radius:10px;width:34px;height:34px;padding:0;font-size:18px;color:#475467;display:inline-grid;place-items:center;line-height:0}.top-panel-close .sb-icon{width:18px;height:18px}.top-panel-close:hover{background:#f1f4f6}
  ...
  function panelHead(title, subtitle){ return `<div class="top-panel-head"><div><h3>${escapeHTML(title)}</h3><p>${escapeHTML(subtitle)}</p></div><button class="top-panel-close" type="button" data-action="close-topbar-panel" aria-label="Close">×</button></div>`; }
  ...
  function smartInsights(){
    ...
    return [
      {id:'balanced', icon:'▣', title:'Accounting done', msg:`Trial balance is ${Math.abs(trialBalance().difference)<0.01?'balanced':'out of balance'}. ${bankReview} bank transactions need review.`, label:'Open accounting', nav:'accounting'},
      ...
      {id:'bank', icon:'◉', title:'Banking review', msg: bankReview ? `${bankReview} bank transaction${bankReview===1?'':'s'} need matching, clearing, or categorization.` : 'Bank feed is reviewed. Reconciliation remains available from Banking.', label:'Review banking', nav:'banking'},
      {id:'cash', icon:'↗', title:'Cash flow insight', msg:`Current bank position is ${money(t.bank)}. Net cash flow is based on recorded payments, deposits, expenses, and bill payments.`, label:'View banking', nav:'banking'},
      {id:'ap', icon:'□', title:'Bills due', msg: dueBills ? `${dueBills} vendor bill${dueBills===1?'':'s'} remain open. Open A/P is ${money(t.ap)}.` : 'No open vendor bills need payment.', label:'Open bills', nav:'expenses'},
      {id:'stock', icon:'◼', title:'Inventory attention', msg: lowStock ? `${lowStock} inventory item${lowStock===1?'':'s'} are at or below reorder point.` : 'Inventory stock levels do not show reorder alerts.', label:'Products & Services', nav:'inventory'},
      {id:'match', icon:'⇄', title:'Purchase matching', msg: matchIssues ? `${matchIssues} purchase order${matchIssues===1?'':'s'} may need receiving, billing, or matching review.` : 'Purchase orders, receiving, and bill matching are in good shape.', label:'3-way match', nav:'inventory'}
    ];
  }
  ...
  function renderSettingsPanel(){
    return panelHead('Settings','Open company, accounting, tax, display, and data controls.')+`<div class="top-panel-body"><div class="panel-list">
      <div class="panel-row" data-modal="company"><div class="panel-row-main"><span class="panel-icon">🏢</span><div><strong>Company profile</strong><span>Name, province, fiscal year, and accounting method.</span></div></div><button class="btn square">Open</button></div>
      <div class="panel-row" data-modal="invoiceCustomize"><div class="panel-row-main"><span class="panel-icon">▤</span><div><strong>Invoice branding</strong><span>Logo, invoice template, message, payment instructions, and print settings.</span></div></div><button class="btn square">Open</button></div>
      <div class="panel-row" data-nav="accounting"><div class="panel-row-main"><span class="panel-icon">≡</span><div><strong>Chart of accounts</strong><span>Accounts, journal entries, trial balance, and audit log.</span></div></div><button class="btn square">Open</button></div>
  ...
  function renderNotificationsPanel(){
    ...
    const rows=appNotifications().map(n=>`<div class="panel-row ${read.has(n.id)?'':'notification-unread'}" data-nav="${n.nav}" data-action="mark-notification-read" data-id="${n.id}"><div class="panel-row-main"><span class="panel-icon">🔔</span><div><strong>${escapeHTML(n.title)}</strong><span>${escapeHTML(n.msg)}</span></div></div><span class="notification-time">${escapeHTML(n.time)}</span></div>`).join('');
  ...
  smartInsights = function(){
    ...
    return [
      {id:'balanced', icon:'▣', title:'Accounting done', msg:`Trial balance is ${Math.abs(trialBalance().difference)<0.01?'balanced':'out of balance'}. ${bankReview} bank transaction${bankReview===1?'':'s'} need review.`, label:'Open accounting', module:'accounting'},
      ...
      {id:'bank', icon:'◉', title:'Banking review', msg: bankReview ? `${bankReview} bank transaction${bankReview===1?'':'s'} need matching, clearing, or categorization.` : 'Bank feed is reviewed. Reconciliation remains available from Banking.', label:'Review banking', module:'banking'},
      {id:'cash', icon:'↗', title:'Cash flow insight', msg:`Current bank position is ${money(t.bank)}. Net cash flow is based on recorded payments, deposits, expenses, and bill payments.`, label:'View banking', module:'banking'},
      {id:'ap', icon:'□', title:'Bills due', msg: dueBills ? `${dueBills} vendor bill${dueBills===1?'':'s'} remain open. Open A/P is ${money(t.ap)}.` : 'No open vendor bills need payment.', label:'Open bills', module:'expenses'},
      {id:'taxes', icon:'◖', title:'Tax return review', msg: taxNet>0 ? `Net sales tax payable is ${money(taxNet)}. Review returns, payments, and adjustments.` : 'Tax Center is ready for return review, input tax credits, and adjustments.', label:'Open taxes', module:'taxes'},
      {id:'stock', icon:'◼', title:'Inventory attention', msg: lowStock ? `${lowStock} inventory item${lowStock===1?'':'s'} are at or below reorder point.` : 'Inventory stock levels do not show reorder alerts.', label:'Products & Services', module:'inventory'},
      {id:'match', icon:'⇄', title:'Purchase matching', msg: matchIssues ? `${matchIssues} purchase order${matchIssues===1?'':'s'} may need receiving, billing, or matching review.` : 'Purchase orders, receiving, and bill matching are in good shape.', label:'3-way match', module:'inventory'}
    ];
  };
  ...
  function v88InsightDefinitions(){
    ...
    return [
      {id:'bank', module:'banking', icon:'◉', title:'Bank transactions need review', msg:d.bankReview ? `${d.bankReview} transaction${d.bankReview===1?'':'s'} need matching, categorization, or clearing.` : 'Bank feed is reviewed. Reconciliation remains available from Banking.', label:'Review banking'},
      ...
      {id:'ap', module:'expenses', icon:'□', title:'Vendor bills open', msg:d.dueBills ? `${d.dueBills} vendor bill${d.dueBills===1?'':'s'} remain open. Open A/P is ${safeV88Money(d.t.ap)}.` : 'No open vendor bills need payment.', label:'Review bills'},
      {id:'taxes', module:'taxes', icon:'◖', title:'Tax return attention', msg:d.taxNet>0 ? `Net sales tax payable is ${safeV88Money(d.taxNet)}. Review returns, payments, and adjustments.` : 'Tax Center is ready for return review, input tax credits, and adjustments.', label:'Open taxes'},
      {id:'stock', module:'inventory', icon:'◼', title:'Inventory attention', msg:d.lowStock ? `${d.lowStock} inventory item${d.lowStock===1?'':'s'} are at or below reorder point.` : 'Inventory stock levels do not show reorder alerts.', label:'Review inventory'},
      {id:'match', module:'inventory', icon:'⇄', title:'Purchase matching', msg:d.matchIssues ? `${d.matchIssues} purchase order${d.matchIssues===1?'':'s'} may need receiving, billing, or matching review.` : 'Purchase orders, receiving, and bill matching are in good shape.', label:'Open matching'},
      {id:'balanced', module:'accounting', icon:'▣', title:'Accounting check', msg:`Trial balance is ${d.tbDiff<0.01?'balanced':'out of balance'}. ${d.bankReview} bank transaction${d.bankReview===1?'':'s'} need review.`, label:'Open accounting'},
      {id:'cash', module:'banking', icon:'↗', title:'Cash flow insight', msg:`Current bank position is ${safeV88Money(d.t.bank)}. Net cash flow is based on recorded payments, deposits, expenses, and bill payments.`, label:'View banking'}
    ];
  }
  ...
  function normalizeV89InsightsButton(){
    ...
    btn.innerHTML='<span class="top-icon-symbol" aria-hidden="true">✦</span>';
    if(existingDot){
  ...
  renderBusinessFeed = function(t){
    ...
    const cards = [
      {icon:'▣', title:'Accounting done', text:`Trial balance is ${tb.ok?'balanced':'out by '+money(Math.abs(tb.diff))}. ${unreviewed} bank transaction${unreviewed===1?'':'s'} need review.`, action:'Open accounting', nav:'accounting'},
      {icon:'!', title:'Overdue invoices', text:`${overdueInvoices.length} invoice${overdueInvoices.length===1?'':'s'} need follow-up. Open receivables are ${money(t.ar)}.`, action:'Review all', nav:'sales'},
      {icon:'◉', title:'Banking review', text:`${unreviewed} imported-style bank transaction${unreviewed===1?'':'s'} need matching, clearing, or categorization.`, action:'Review banking', nav:'banking'},
      {icon:'↗', title:'Cash flow insight', text:`Operating Checking is ${money(cash.operatingBalance)}. Net cash position across bank and credit accounts is ${money(cash.netPosition)}.`, action:'View banking', nav:'banking'},
      {icon:'◼', title:'Low stock detected', text: lowStock ? `${lowStock} inventory item${lowStock===1?'':'s'} are at or below reorder point.` : 'Inventory stock levels are within reorder rules.', action:'Open items', nav:'inventory', inventoryOnly:true},
      {icon:'◖', title:'Tax return ready', text: tax.balanceMessage, action:'Prepare return', nav:'taxes'},
      {icon:'▸', title:'Bills due', text:`${dueBills.length} open vendor bill${dueBills.length===1?'':'s'} require review or payment scheduling.`, action:'Review bills', nav:'expenses'}
  ...
  function v810InsightDefinitions(){
    ...
    const tb=trialBalanceStatus();
    const items=[
      {id:'bank', module:'banking', icon:'◉', title:'Bank transactions need review', msg:`${bankReview} transaction${bankReview===1?'':'s'} need matching, categorization, or clearing.`, label:'Review banking'},
      ...
      {id:'ap', module:'expenses', icon:'□', title:'Vendor bills open', msg:`${dueBills.length} vendor bill${dueBills.length===1?'':'s'} remain open. Open A/P is ${money(t.ap)}.`, label:'Review bills'},
      {id:'taxes', module:'taxes', icon:'◖', title:'Tax return attention', msg:tax.balanceMessage, label:'Open taxes'},
      {id:'balanced', module:'accounting', icon:'▣', title:'Accounting check', msg:`Trial balance is ${tb.ok?'balanced':'out of balance'}. ${bankReview} bank transaction${bankReview===1?'':'s'} need review.`, label:'Open accounting'},
      {id:'cash', module:'banking', icon:'↗', title:'Cash flow insight', msg:`Operating Checking is ${money(cash.operatingBalance)}. Net cash position across checking, savings, and credit card accounts is ${money(cash.netPosition)}.`, label:'View banking'}
    ];
    if(v810CanShowInventoryInsights()){
      items.splice(4,0,
        {id:'stock', module:'inventory', icon:'◼', title:'Inventory attention', msg:lowStock.length ? `${lowStock.length} inventory item${lowStock.length===1?'':'s'} are at or below reorder point.` : 'Inventory stock levels are within reorder rules.', label:'Review inventory'},
        {id:'match', module:'inventory', icon:'⇄', title:'Purchase matching', msg:matchIssues.length ? `${matchIssues.length} purchase order${matchIssues.length===1?'':'s'} may need receiving, billing, or matching review.` : 'Purchase orders, receiving, and bill matching are in good shape.', label:'Open matching'}
      );
    }
    return items;
  }
  ...
  reportDataV81 = function(id){
    ...
    data.subtitle = `${data.subtitle || ''}${start||end ? ` · Date range: ${start||'Beginning'} to ${end||'Today'}` : ''}`;
  ...
  dashboardShortcutDefaults = ['sales','expenses','banking','reports','customers','inventory','taxes','projects','time','accounting','transactions'];
  ...
  function renderModulePillsV814(){
    ...
    const el=document.getElementById('modulePills');
    if(el){
      el.innerHTML = `<div class="shortcut-shell"><button class="shortcut-arrow" type="button" data-action="shortcut-scroll" data-dir="-1" aria-label="Previous shortcuts">‹</button><div class="shortcut-viewport"><div class="shortcut-track" id="shortcutTrack">${mods.map(m=>`<button class="module-pill ${currentPage===m.id?'active':''}" data-nav="${m.id}"><span class="module-icon">${m.icon}</span>${escapeHTML(m.label)}</button>`).join('')}</div></div><button class="shortcut-arrow" type="button" data-action="shortcut-scroll" data-dir="1" aria-label="More shortcuts">›</button><button class="shortcut-customize" type="button" data-modal="customizeShortcuts" title="Customize shortcuts" aria-label="Customize shortcuts">⚙</button></div>`;
  ...
  renderDashboard = function(){
    ...
    page.innerHTML = `
      <div class="hero v8-hero"><h2 id="greeting">Welcome back</h2><div class="pill-row" id="modulePills"></div></div>
      <div class="feed-header"><h3>✦ Business Feed</h3><button class="btn soft" data-action="view-all-feed">View all</button></div>
      <div id="businessFeed"></div>
      ${quickActionsV814()}
      <div class="section-header"><div><h2>Business at a glance</h2><p>Clean summary of receivables, deposits, bank balances, profit, expenses, and setup items.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-modal="customizeDashboard">⚙ Customize</button><button class="btn" data-action="toggle-privacy">◉ Privacy</button><button class="btn square" data-action="refresh-dashboard">↻ Refresh</button></div></div>
      <div class="v8-dashboard-grid">
  ...
  function businessFeedFullItems(){
    ...
    return items.length?items:[{id:'none',icon:'✓',title:'No urgent insights right now',msg:'Your business data looks up to date.',label:'Open dashboard',module:'dashboard',category:'General'}];
  }
  ...
  v815FeedCategories = [
    {id:'banking', label:'Banking', icon:'◉', desc:'Bank-feed review, matching, clearing, and reconciliation.'},
    {id:'sales', label:'Sales / invoices', icon:'↗', desc:'Open receivables, overdue invoices, payments, and estimates.'},
    {id:'expenses', label:'Expenses / bills', icon:'▸', desc:'Vendor bills, payments, approvals, and due-date reminders.'},
    {id:'taxes', label:'Taxes', icon:'◖', desc:'GST/HST return, credit/payable, and tax-code review.'},
    {id:'inventory', label:'Products & Services', icon:'◼', desc:'Low stock, inventory attention, and item setup.'},
    {id:'matching', label:'Purchase matching', icon:'⇄', desc:'PO, receiving, vendor bill, and three-way match alerts.'},
    {id:'accounting', label:'Accounting', icon:'▤', desc:'Trial balance, journal review, and accounting checks.'},
    {id:'cash', label:'Cash flow', icon:'$', desc:'Cash position, net movement, and bank balances.'},
    {id:'setup', label:'Setup / admin', icon:'✓', desc:'Company setup, preferences, and backup reminders.'}
  ];
  ...
  v815DefaultFeedCategories = v815FeedCategories.map(c=>c.id);
  ...
  function bookmarkCatalog(){
    ensureV815State();
    const base = [
      {id:'dashboard', label:'Dashboard', icon:'⌂', nav:'dashboard', desc:'Main business overview.'},
      {id:'reports-favorites', label:'Favorite reports', icon:'↗', nav:'reports', desc:'Reports & Analytics favorites.'},
      {id:'banking', label:'Banking', icon:'◉', nav:'banking', desc:'Banking Center and reconciliation.'},
      {id:'sales', label:'Sales & Get Paid', icon:'↗', nav:'sales', desc:'Invoices, payments, and sales orders.'},
      {id:'customers', label:'Customers & Leads', icon:'☘', nav:'customers', desc:'Customer profiles and open balances.'},
      {id:'expenses', label:'Expenses & Pay Bills', icon:'▸', nav:'expenses', desc:'Bills, expenses, and AP workflow.'},
      {id:'vendors', label:'Vendors', icon:'□', nav:'vendors', desc:'Supplier profiles and purchase activity.'},
      {id:'reports', label:'Reports', icon:'☷', nav:'reports', desc:'Reports & Analytics center.'},
      {id:'inventory', label:'Products & Services', icon:'◼', nav:'inventory', desc:'Services, inventory, POs, and sales orders.'},
      {id:'taxes', label:'Taxes', icon:'◖', nav:'taxes', desc:'Tax Center and returns.'},
      {id:'accounting', label:'Accounting', icon:'▤', nav:'accounting', desc:'Chart of accounts and trial balance.'},
      {id:'transactions', label:'Transactions', icon:'⇄', nav:'transactions', desc:'All transaction activity.'},
      {id:'projects', label:'Projects', icon:'◆', nav:'projects', desc:'Project budgets and profitability.'},
      {id:'time', label:'Time', icon:'◷', nav:'time', desc:'Time entries and billable work.'},
      {id:'payroll', label:'Payroll & HR', icon:'♢', nav:'payroll', desc:'Payroll and employee setup.'},
      {id:'setup', label:'Setup Checklist', icon:'✓', nav:'setup', desc:'Guided setup tasks.'},
      {id:'apps', label:'My Apps', icon:'▩', nav:'apps', desc:'Enabled module launcher.'},
      {id:'settings', label:'Settings', icon:'⚙', nav:'settings', desc:'Company and app controls.'}
    ];
  ...
  businessFeedFullItems = function(){
    ...
    return items.length?items:[{id:'none',icon:'✓',title:'No feed cards selected',msg:'Choose feed categories from Customize feed to show alerts and insights.',label:'Customize feed',module:'dashboard',nav:'dashboard',categoryId:'setup',category:'Setup / admin'}];
  };
  ...
  v816FeedCategories = [
    {id:'banking', label:'Banking', icon:'◉', desc:'Bank review, matching, clearing, and reconciliation.', module:'banking'},
    {id:'sales', label:'Sales & Invoices', icon:'↗', desc:'Open receivables, overdue invoices, payments, and estimates.', module:'sales'},
    {id:'expenses', label:'Expenses & Bills', icon:'▸', desc:'Vendor bills, payments, approvals, and due-date reminders.', module:'expenses'},
    {id:'taxes', label:'Taxes', icon:'◖', desc:'GST/HST return, credit/payable, and tax-code review.', module:'taxes'},
    {id:'inventory', label:'Products & Services', icon:'◼', desc:'Low stock, inventory attention, and item setup.', module:'inventory', requiresInventoryMode:true},
    {id:'matching', label:'Purchase Matching', icon:'⇄', desc:'PO, receiving, vendor bill, and three-way match alerts.', module:'inventory', requiresPurchaseMode:true},
    {id:'accounting', label:'Accounting', icon:'▤', desc:'Trial balance, journal review, and accounting checks.', module:'accounting'},
    {id:'cash', label:'Cash Flow', icon:'$', desc:'Cash position, net movement, and bank balances.', module:'banking'},
    {id:'setup', label:'Setup & Admin', icon:'✓', desc:'Company setup, preferences, and backup reminders.', module:'settings'}
  ];
  v816CategoryMap = {bank:'banking', overdue:'sales', ap:'expenses', taxes:'taxes', stock:'inventory', match:'matching', balanced:'accounting', cash:'cash', none:'setup'};
  v816DefaultCategories = v816FeedCategories.map(c=>c.id);
  ...
  businessFeedFullItems = function(){
    ...
    return items.length ? items : [{id:'none',icon:'✓',title:'No urgent insights right now',msg:'Your visible Business Feed categories are up to date.',label:'Customize feed',module:'dashboard',nav:'dashboard',categoryId:'setup',category:'Setup & Admin'}];
  };
  ...
  function dashboardFeedDisplayLabel(){
    normalizeFeedDisplayMode();
    return state.settings.dashboardFeedDisplay==='expanded' ? 'Expanded — show all active cards' : state.settings.dashboardFeedDisplay==='standard' ? 'Standard — show up to 6 cards' : 'Compact — show top 4 cards';
  }
  ...
  function updateBusinessFeedHeaderCount(){
    ...
    if(titleWrap){
      ...
      note.textContent = all.length ? `Showing ${shown.length} of ${all.length} insights · ${dashboardFeedDisplayLabel()}` : 'No active insights selected';
    }
    const view=header.querySelector('[data-action="view-all-feed"]');
  ...
  function renderBusinessFeedCardsWithDismiss(){
    ...
    feed.innerHTML=`<div class="feed-row">${items.map(x=>`<div class="feed-card" data-action="open-insight" data-id="${escapeHTML(x.id)}" role="button" tabindex="0"><button class="hide-feed-btn" type="button" data-action="hide-feed-card" data-id="${escapeHTML(x.id)}" title="Hide this insight" aria-label="Hide ${escapeHTML(x.title)}">×</button><div class="feed-title"><span class="feed-badge">${escapeHTML(x.icon)}</span>${escapeHTML(x.title)}</div><p>${escapeHTML(x.msg)}</p><button class="btn soft" type="button" data-action="open-insight" data-id="${escapeHTML(x.id)}">${escapeHTML(x.label)}</button></div>`).join('')}</div>`;
    updateBusinessFeedHeaderCount();
  }
  ...
  customizeFeedBody = function(){
    ...
    const displayPanel=`<section class="feed-display-panel"><h4 style="margin:0">Dashboard Display</h4><p class="muted small" style="margin:4px 0 0">Choose how many active cards appear on the dashboard. View all still shows every active card.</p><div class="feed-display-options">
      <label><input type="radio" name="dashboardFeedDisplay" value="compact" ${state.settings.dashboardFeedDisplay==='compact'?'checked':''}>Compact · 4 cards</label>
      <label><input type="radio" name="dashboardFeedDisplay" value="standard" ${state.settings.dashboardFeedDisplay==='standard'?'checked':''}>Standard · 6 cards</label>
      <label><input type="radio" name="dashboardFeedDisplay" value="expanded" ${state.settings.dashboardFeedDisplay==='expanded'?'checked':''}>Expanded · all cards</label>
    </div></section>`;
  ...
  dashboardFeedDisplayLabel = function(){
    ...
    if(mode==='custom') return `Custom view — show ${state.settings.dashboardFeedCustomCount} cards`;
    return 'Compact view';
  };
  ...
  updateBusinessFeedHeaderCount = function(){
    ...
    if(note) note.textContent = all.length ? `Showing ${shown.length} of ${all.length} insights · ${dashboardFeedDisplayLabel()}` : 'No active insights selected';
    const view=header.querySelector('[data-action="view-all-feed"]');
  ...
  customizeFeedBody = function(){
    ...
    const displayPanel=`<section class="feed-display-panel"><h4 style="margin:0">Dashboard Display</h4><p class="muted small" style="margin:4px 0 0">Choose how many active insight cards appear on the dashboard. View all still shows every active card.</p><div class="feed-display-options">
      <label><input type="radio" name="dashboardFeedDisplay" value="compact" ${state.settings.dashboardFeedDisplay==='compact'?'checked':''}>Compact · 3 cards</label>
      <label><input type="radio" name="dashboardFeedDisplay" value="standard" ${state.settings.dashboardFeedDisplay==='standard'?'checked':''}>Standard · 6 cards</label>
      <label><input type="radio" name="dashboardFeedDisplay" value="expanded" ${state.settings.dashboardFeedDisplay==='expanded'?'checked':''}>Expanded · all cards</label>
      <label><input type="radio" name="dashboardFeedDisplay" value="custom" ${state.settings.dashboardFeedDisplay==='custom'?'checked':''}>Custom · <input type="number" name="dashboardFeedCustomCount" min="1" max="24" value="${num(state.settings.dashboardFeedCustomCount||3)}" style="width:72px;margin-left:4px"> cards</label>
    </div></section>`;
  ...
  modalBody = function(type){
    if(type==='invoiceEdit'){
      ...
      const lines=(inv.items||[]).map((line,idx)=>`<div class="invoice-line-edit-row"><div><strong>${escapeHTML(line.product||line.productId||'Line item')}</strong><div class="muted small">Qty ${num(line.qty)} · Rate ${money(num(line.rate))} · Amount ${money(num(line.amount ?? num(line.qty)*num(line.rate)))}</div></div><div class="field"><label>Line-item description</label><textarea name="lineDesc_${idx}">${escapeHTML(line.desc||'')}</textarea></div></div>`).join('') || '<div class="muted">No line items found.</div>';
  ...
  function renderCashFlowHero(){
    ...
    el.innerHTML=`<div class="cash-hero-head"><div class="cash-hero-title"><div class="eyebrow">Cash Flow</div><div class="muted small">Last 12 months</div><div class="cash-balance">${money(balance)}</div><div class="cash-caption">Operating cash balance</div></div><div class="cash-toggle" role="tablist" aria-label="Cash flow chart view"><button type="button" class="${view==='flow'?'active':''}" data-action="cashflow-view" data-id="flow">▥ Money in/out</button><button type="button" class="${view==='balance'?'active':''}" data-action="cashflow-view" data-id="balance">⌁ Cash balance</button></div></div><div class="cash-chart-wrap"><div class="cash-chart">${bars}</div></div><div class="cash-legend">${view==='balance'?`<span><i class="cash-dot balance"></i>Cash balance</span>`:`<span><i class="cash-dot in"></i>Money in</span><span><i class="cash-dot out"></i>Money out</span>`}</div><button class="cash-source-link" type="button" data-nav="banking">Where do these numbers come from?</button>`;
  }
  ...
  renderDashboard = function(){
    ...
    page.innerHTML = `
      ...
      <div class="dashboard-feed-block" id="businessFeedBlock"><div class="feed-header"><h3>✦ Business Feed</h3><button class="btn soft" data-action="view-all-feed">View all insights</button></div><div id="businessFeed"></div></div>
      <div class="section-header"><div><h2>Additional insights</h2><p>Profitability, recent transactions, setup items, and optional widgets.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-modal="customizeDashboard">⚙ Customize</button><button class="btn" data-action="toggle-privacy">◉ Privacy</button><button class="btn square" data-action="refresh-dashboard">↻ Refresh</button></div></div>
