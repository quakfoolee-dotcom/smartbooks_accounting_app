...
// Loaded by frontend/index.html in dependency order.
  ...
  // ---------- V33 Base Global Smart Search ----------
  // Moved from dashboard-widgets.js so search behavior has a single feature owner.
  ...
  function v42ActionButtons(page){
    const actions={
      ...
      transactions:`<button class="btn primary" data-open-create>＋ New</button><button class="btn" data-nav="reports">Open reports</button>`,
    ...
    return actions[page] || `<button class="btn primary" data-open-create>＋ New</button><button class="btn" data-nav="dashboard">Back to dashboard</button>`;
  }
  ...
  function v43ActionButtons(page){
    ...
    return `<button class="btn primary" data-open-create>＋ New</button><button class="btn" data-nav="dashboard">Back to dashboard</button>`;
  }
  ...
  function v44ShellActions(page){
    ...
    return typeof v43ActionButtons==='function' ? v43ActionButtons(page) : '<button class="btn primary" data-open-create>＋ New</button>';
  }
  ...
  function v47AccountLabel(id){ try{return accountLabel(id);}catch(e){ const a=(state.chartOfAccounts||[]).find(x=>x.id===id||x.code===id); return a?`${a.code} · ${a.name}`:(id||''); } }
  ...
  function v47BuildSearchIndex(){
    ...
    v47Add(rows,'Pages','Dashboard','Business overview, widgets, cash flow and quick actions.','Page','⌂','home business overview',{kind:'nav',page:'dashboard'},15,[{label:'Open',action:'open',primary:true}]);
    ...
    actions.forEach(([title,desc,modal,keywords])=>v47Add(rows,'Actions',title,desc,'Action','＋',keywords,{kind:'modal',modal},22,[{label:'Open',action:'open',primary:true}]));

    try{ const reports=(typeof reportCatalogV8==='function') ? reportCatalogV8() : []; reports.forEach(r=>{ let aliases=''; if(r.id==='profit-loss') aliases='p&l pnl profit loss income statement net income revenue expenses'; if(r.id==='ar-aging') aliases='ar a/r accounts receivable who owes you aging unpaid overdue open invoices'; if(r.id==='ap-aging') aliases='ap a/p accounts payable what you owe aging unpaid bills vendors'; if(r.id==='sales-tax-liability') aliases='gst hst tax cra sales tax liability remittance'; if(r.id==='balance-sheet') aliases='assets liabilities equity financial position'; if(r.id==='cash-flow') aliases='cash flow bank money in money out'; v47Add(rows,'Reports',r.name,r.desc||r.category,'Report','▤',`${r.id} ${r.category} ${aliases}`,{kind:'report',reportId:r.id},20,[{label:'Open report',action:'open',primary:true}]); }); }catch(e){}

    (state.invoices||[]).forEach(i=>{ const c=v47Customer(i.customerId), cname=c.name||i.customerId||'', total=v47InvTotal(i), open=v47InvOpen(i), st=(typeof invoiceDisplayStatus==='function'?invoiceDisplayStatus(i):i.status)||'Invoice'; const itemText=(i.items||[]).map(x=>`${x.desc||''} ${x.productId||''}`).join(' '); const actions=[{label:'View',action:'view',primary:true}]; if(open>0.01) actions.push({label:'Receive payment',action:'receive'}); actions.push({label:(i.sentDate?'Send reminder':'Send'),action:'send'},{label:'Print/PDF',action:'print'}); v47Add(rows,'Transactions',`${i.id} · ${cname}`,`${st} · ${i.date||''} · Due ${i.dueDate||'—'} · Total ${v47Money(total)} · Open ${v47Money(open)}`,'Invoice','INV',`${i.id} ${cname} ${c.company||''} ${c.email||''} ${st} ${i.date||''} ${i.dueDate||''} ${total} ${open} ${i.memo||''} ${itemText} ${v47AccountLabel(i.incomeAccountId||'4000')} accounts receivable open overdue paid sent`,{kind:'invoice',id:i.id},38,actions); });
    (state.payments||[]).forEach(p=>{ const cname=v47CustomerName(p.customerId); const actions=[{label:'View',action:'view',primary:true}]; if(p.invoiceId) actions.push({label:'Open invoice',action:'invoice'}); if(!p.depositId) actions.push({label:'Deposit',action:'deposit'}); v47Add(rows,'Transactions',`${p.id} · ${cname}`,`Payment · ${p.date||''} · ${v47Money(p.amount)}${p.invoiceId?' · '+p.invoiceId:''}`,'Payment','$',`${p.id} ${cname} ${p.invoiceId||''} ${p.amount} ${p.date||''} received paid deposit ${p.memo||''}`,{kind:'payment',id:p.id},32,actions); });
    (state.estimates||[]).forEach(e=>{ const cname=v47CustomerName(e.customerId); const numId=e.estimateNumber||e.id; const total=v47Num(e.total ?? e.amount); const st=(typeof v18EstimateDisplayStatus==='function'?v18EstimateDisplayStatus(e):e.status)||'Estimate'; const actions=[{label:'View',action:'view',primary:true},{label:'Edit',action:'edit'}]; if(st==='Accepted' && !e.convertedInvoiceId) actions.push({label:'Convert',action:'convert'}); v47Add(rows,'Transactions',`${numId} · ${cname}`,`${st} · ${e.date||''} · ${v47Money(total)}${e.convertedInvoiceId?' · Invoice '+e.convertedInvoiceId:''}`,'Estimate','EST',`${e.id} ${e.estimateNumber||''} ${cname} quote estimate proposal pipeline ${st} ${e.date||''} ${e.expiryDate||''} ${total} ${e.scope||''} ${e.terms||''}`,{kind:'estimate',id:e.id},32,actions); });
    (state.expenses||[]).forEach(x=>{ const vname=v47VendorName(x.vendorId); const total=v47Num(x.total ?? x.amount)+v47Num(x.tax); v47Add(rows,'Transactions',`${x.id} · ${vname}`,`Expense · ${x.date||''} · ${v47Money(total)} · ${x.memo||x.account||''}`,'Expense','EXP',`${x.id} ${vname} ${x.memo||''} ${x.paymentMethod||''} ${total} ${x.date||''} ${v47AccountLabel(x.expenseAccountId||'')} ${x.account||''}`,{kind:'expense',id:x.id},28,[{label:'View',action:'view',primary:true}]); });
    (state.bills||[]).forEach(b=>{ const vname=v47VendorName(b.vendorId); const open=v47BillOpen(b); const actions=[{label:'View',action:'view',primary:true}]; if(open>0.01) actions.push({label:'Pay bill',action:'pay'}); v47Add(rows,'Transactions',`${b.id} · ${vname}`,`${b.status||'Bill'} · Due ${b.dueDate||'—'} · Total ${v47Money(v47BillTotal(b))} · Open ${v47Money(open)}`,'Bill','BILL',`${b.id} ${vname} ${b.status||''} ${b.dueDate||''} ${open} ${v47BillTotal(b)} accounts payable ap vendor bill pay due ${b.memo||''}`,{kind:'bill',id:b.id},30,actions); });
    (state.deposits||[]).forEach(d=>v47Add(rows,'Transactions',`${d.id} · Deposit`,`${d.date||''} · ${v47Money(d.amount)} · ${d.memo||''}`,'Deposit','DEP',`${d.id} bank deposit ${d.amount} ${d.date||''} ${d.memo||''}`,{kind:'deposit',id:d.id},26,[{label:'View',action:'view',primary:true}]));
    (state.bankTransactions||[]).forEach(tx=>v47Add(rows,'Transactions',`${tx.id} · ${tx.description}`,`${tx.date||''} · ${v47Money(tx.amount)} · ${tx.status||'Bank feed'} · ${tx.matchType||''}`,'Bank feed','BANK',`${tx.id} ${tx.description} ${tx.amount} ${tx.date||''} ${tx.note||''} bank review reconcile match categorize clear ${tx.linkedId||''}`,{kind:'banktx',id:tx.id},30,[{label:'View',action:'view',primary:true},{label:'Review',action:'review'}]));

    (state.customers||[]).forEach(c=>{ const open=(typeof customerOpenBalance==='function'?customerOpenBalance(c.id):0); v47Add(rows,'Customers',c.name,`${c.company||''} · ${c.email||''} · Open balance ${v47Money(open)}`,'Customer','👥',`${c.id} ${c.name||''} ${c.company||''} ${c.email||''} ${c.phone||''} ${c.type||''} receivable invoice estimate customer`,{kind:'customer',id:c.id},28,[{label:'View',action:'view',primary:true},{label:'Invoice',action:'invoice'},{label:'Estimate',action:'estimate'}]); });
    (state.vendors||[]).forEach(v=>v47Add(rows,'Vendors',v.name,`${v.category||'Vendor'} · ${v.email||''} · ${v.phone||''}`,'Vendor','🏪',`${v.id} ${v.name||''} ${v.email||''} ${v.phone||''} ${v.category||''} bill supplier payable`,{kind:'vendor',id:v.id},28,[{label:'View',action:'view',primary:true},{label:'Create bill',action:'bill'}]));
    (state.products||[]).forEach(p=>v47Add(rows,'Products & Services',p.name,`${p.type||'Item'} · ${v47Money(p.price)} · ${v47AccountLabel(p.incomeAccountId||'')}`,'Item','▣',`${p.id} ${p.name||''} ${p.sku||''} ${p.type||''} ${p.price} product service sku inventory item revenue ${v47AccountLabel(p.incomeAccountId||'')}`,{kind:'product',id:p.id},24,[{label:'View',action:'view',primary:true}]));
    (state.chartOfAccounts||[]).forEach(a=>v47Add(rows,'Accounting',`${a.code} · ${a.name}`,`${a.type} · ${a.detail||''} · Balance ${v47Money((typeof normalBalance==='function')?normalBalance(a.id):0)}`,'Account','▤',`${a.id} ${a.code} ${a.name||''} ${a.type||''} ${a.normal||''} ${a.detail||''} chart of accounts ledger debit credit balance`,{kind:'account',id:a.id},22,[{label:'View',action:'view',primary:true},{label:'Ledger',action:'ledger'}]));

    v47Add(rows,'Help','Search tips','Use invoice numbers, customer names, vendor names, amounts, due dates, report names, or commands such as “new invoice”.','Help','?','help keyboard search tips exact match fuzzy search commands',{kind:'help',id:'search-tips'},12,[{label:'Open help',action:'open',primary:true}]);
  ...
  function v47CandidateResults(query){
    ...
    if(!q){
      ...
      const recentSearches=(state.settings?.v47RecentSearches||[]).slice(0,4).map(s=>({group:'Recent',title:`Search again: ${s}`,desc:'Recent search',badge:'Recent',icon:'⌕',path:'Recent searches',keywords:v47Lower(s),norm:v47Norm(s),target:{kind:'search',query:s},priority:42,actions:[{label:'Search',action:'open',primary:true}]}));
  ...
  function v47NoResultsHTML(q){
    return `<div class="v47-search-head"><div class="v47-search-titlebar"><strong>No matching result</strong><span>Try a record ID, name, amount, report, or command</span></div>${v47FilterChipsHTML(q)}</div><div class="v47-search-empty"><h4>No result for “${escapeHTML(q)}”</h4><div>Search checks invoices, payments, estimates, bills, expenses, customers, vendors, products, accounts, reports, and actions.</div><div class="v47-search-tips"><button class="v47-search-tip primary" data-v47-fallback="create-invoice">Create invoice</button><button class="v47-search-tip" data-v47-tip="invoice">invoice</button><button class="v47-search-tip" data-v47-tip="AR aging">AR aging</button><button class="v47-search-tip" data-v47-tip="bank review">bank review</button><button class="v47-search-tip" data-v47-fallback="transactions">Search all transactions</button></div></div>`;
  }
  ...
  function v47PaymentDetail(id){ const p=(state.payments||[]).find(x=>x.id===id); if(!p) return v47DetailMissing('Payment',id); return `<div class="v47-detail-grid"><div class="v47-detail-card"><h3>Payment</h3>${v47Line('Payment ID',p.id)}${v47Line('Customer',v47CustomerName(p.customerId))}${v47Line('Invoice',p.invoiceId||'—')}${v47Line('Date',p.date||'')}${v47Line('Amount',v47Money(p.amount))}</div><div class="v47-detail-card"><h3>Deposit status</h3>${v47Line('Account',v47AccountLabel(p.accountId||p.depositedToAccountId||''))}${v47Line('Deposit ID',p.depositId||'Undeposited')}${v47Line('Deposited date',p.depositedDate||'—')}<p class="muted small">${escapeHTML(p.memo||'No memo')}</p></div></div>`; }
  function v47BillDetail(id){ const b=(state.bills||[]).find(x=>x.id===id); if(!b) return v47DetailMissing('Bill',id); return `<div class="v47-detail-grid"><div class="v47-detail-card"><h3>Bill</h3>${v47Line('Bill ID',b.id)}${v47Line('Vendor',v47VendorName(b.vendorId))}${v47Line('Bill date',b.date||'—')}${v47Line('Due date',b.dueDate||'—')}${v47Line('Status',b.status||'Bill')}</div><div class="v47-detail-card"><h3>Amounts</h3>${v47Line('Subtotal',v47Money(b.amount||b.subtotal||0))}${v47Line('Tax',v47Money(b.tax||0))}${v47Line('Total',v47Money(v47BillTotal(b)))}${v47Line('Open',v47Money(v47BillOpen(b)))}</div></div>`; }
  function v47ExpenseDetail(id){ const x=(state.expenses||[]).find(e=>e.id===id); if(!x) return v47DetailMissing('Expense',id); const total=v47Num(x.total ?? x.amount)+v47Num(x.tax); return `<div class="v47-detail-grid"><div class="v47-detail-card"><h3>Expense</h3>${v47Line('Expense ID',x.id)}${v47Line('Vendor',v47VendorName(x.vendorId))}${v47Line('Date',x.date||'')}${v47Line('Payment method',x.paymentMethod||'—')}${v47Line('Total',v47Money(total))}</div><div class="v47-detail-card"><h3>Posting context</h3>${v47Line('Expense account',v47AccountLabel(x.expenseAccountId||x.account||''))}${v47Line('Tax',v47Money(x.tax||0))}<p class="muted small">${escapeHTML(x.memo||'No memo')}</p></div></div>`; }
  ...
  v47CandidateResults = function(query){
    ...
    if(scope){
      results=results.filter(scope.match).map(r=>({...r,desc:`${scope.label} match · ${r.desc}`}));
    }
    return results;
  };
  ...
  function v48EnhanceSearchDropdown(query){
    ...
    if(titlebar && scope){ titlebar.innerHTML=`${escapeHTML(scope.label)} search for “${escapeHTML(scope.query)}”`; }
  }
