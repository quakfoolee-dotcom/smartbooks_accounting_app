...
| Backend startup | 20,000 ms |
| Backend save | 5,000 ms |
| Backend API reads | 3,000 ms |
| Large-state local startup | 45,000 ms |
| Large-state backend startup | 45,000 ms |
| Large-state backend read | 6,000 ms |
| Large-state backend save | 12,000 ms |
| Large-state dashboard render | 8,000 ms |
| Large-state navigation step | 8,000 ms |

Budgets use the local profile by default and the CI profile when `CI` is set. Any budget can be overridden with a profile-specific variable such as `SMARTBOOKS_LARGE_STARTUP_BUDGET_CI_MS`; the legacy non-profile variable still works, such as `SMARTBOOKS_LARGE_STARTUP_BUDGET_MS`.

```powershell
$env:SMARTBOOKS_STARTUP_BUDGET_MS="8000"
$env:SMARTBOOKS_BACKEND_STARTUP_BUDGET_MS="12000"
$env:SMARTBOOKS_BACKEND_SAVE_BUDGET_MS="2500"
$env:SMARTBOOKS_API_READ_BUDGET_MS="1500"
$env:SMARTBOOKS_LARGE_STARTUP_BUDGET_MS="28000"
$env:SMARTBOOKS_LARGE_BACKEND_STARTUP_BUDGET_MS="30000"
$env:SMARTBOOKS_LARGE_BACKEND_READ_BUDGET_MS="4000"
$env:SMARTBOOKS_LARGE_BACKEND_SAVE_BUDGET_MS="7000"
$env:SMARTBOOKS_LARGE_DASHBOARD_RENDER_BUDGET_MS="4000"
...
- backend read timing and response size
- backend save timing
